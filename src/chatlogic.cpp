...
ChatLogic::ChatLogic()
{
    ...
    //_chatBot = new ChatBot("/workspace/home/CppND-Memory-Management-Chatbot/images/chatbot.png");
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            _ownedEdges.push_back(std::move(edge));
                            // find all keywords for current node
                            AddAllTokensToElement("KEYWORD", tokens, *_edges.back());
                            // store reference in child node and parent node
                            (*childNode)->AddEdgeToChildNode(std::move(_edges.back()));
                            (*parentNode)->AddEdgeToParentNode(std::move(_edges.back())); 
                        }
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        //std::cout << "(*it)->GetNumberOfParents()" << (*it)->GetNumberOfParents() << std::endl;
        
        GraphNode* nodePtr = it->get();
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            if (rootNode == nullptr)
            {
                rootNode = nodePtr; // assign current node to root
            }
            else
            {
    ...
    ChatBot chatBot("/workspace/home/CppND-Memory-Management-Chatbot/images/chatbot.png");
